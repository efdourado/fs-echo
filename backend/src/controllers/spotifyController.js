...
export const spotifyCallback = async (req, res) => {
    ...
    try {
        ...
        const { id: spotifyId, display_name, email, images } = userProfileResponse.data;
        ...
        if (!user) {
            user = new User({
                ...
                email: email,
                password: `spotify:${spotifyId}`, // Placeholder password for Spotify users
...
export const getFeaturedPlaylists = async (req, res) => {
    try {
        ...
        const message = error.response ? (error.response.data?.error?.message || 'An error occurred') : 'A server error occurred while contacting Spotify.';
