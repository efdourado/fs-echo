...
const CollectionPage = ({ type }) => {
  ...
  const handleDelete = async () => {
    if (window.confirm(`Are you sure you want to delete the playlist "${collection.name}"?`)) {
      try {
        await deletePlaylist(id);
        alert("Playlist deleted successfully!");
        navigate("/library");
      } catch (err) {
        setError(err.response?.data?.message || "Failed to delete the playlist.");
      }
    }
  };
  ...
  const isOwner = type === "playlist" && currentUser && collection && collection.owner?._id === currentUser._id;

  const handlePlayCollection = () => {
    const tracks = type === "playlist" ? collection.songs.map((item) => item.song).filter(Boolean) : collection.songs.filter(Boolean);
  ...
  return (
    <>
      <div className="collection-page-container">
        <header className="collection-page-hero">
          <div
            className="collection-hero-bg"
            style={{ backgroundImage: `url(${displayData.coverImage})` }}
          />
        ...
        <div className="collection-page-content">
            <div className="collection-actions">
                ...
                <FontAwesomeIcon
                    icon={isThisCollectionPlaying && isPlaying ? faPauseCircle : faPlayCircle}
                />
                <span>{isThisCollectionPlaying && isPlaying ? "Pause" : "Play"}</span>
                </button>
                {isOwner && (
                <>
                    <button className="play-button-large" onClick={() => setEditModalOpen(true)}>
                    <FontAwesomeIcon icon={faEdit} />
                    <span>Edit</span>
                    </button>
                    <button className="play-button-large" onClick={handleDelete} style={{ backgroundColor: "var(--color-error)" }}>
